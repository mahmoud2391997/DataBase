...
-- CANNOT DELETE A RECORD THAT IS USED AS A FORIGN KEY IN ANOTHER TABLE
DROP TABLE PM.PROJECTS;
TRUNCATE TABLE PM.PROJECTS;
-- SUBQUERY
UPDATE PM.Projects SET InitialCost = InitialCost * 1.05 WHERE PRJNO IN (SELECT PRJNO FROM PM.ProjectTechnologies WHERE TechnologyId= (SELECT Id FROM PM.Technologies WHERE Name = 'ORACLE'));
